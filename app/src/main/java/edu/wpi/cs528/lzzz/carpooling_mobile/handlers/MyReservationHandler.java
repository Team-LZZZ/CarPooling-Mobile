...
public class MyReservationHandler implements IConnectionAsyncTaskDelegate {

    private final String TAG = "MyReservationHandler";
    private IConnectionStatus ConnectionStatus;

    public MyReservationHandler(IConnectionStatus connectionStatus){
    ...
    public void processResult(HttpResponseMessage response) {
        ...
        try {
            ...
            if(!isSuccessful){
                ...
                AppContainer.getInstance().getMyReservations().clear();
                for (String carPoolJson : carPoolJsonArray){
                    CarPool carPool = gson.fromJson(carPoolJson, CarPool.class);
                    AppContainer.getInstance().getMyReservations().add(carPool);
                }
            }
